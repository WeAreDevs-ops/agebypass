...
const API_DOMAINS = [
    ...
    'twostepverification.roblox.com', 'accountsettings.roblox.com', 'api.roblox.com'
];
...
async function changeBirthdateViaUI(cookie, password, birthMonth, birthDay, birthYear, logs) {
    ...
    try {
        ...
        // 1. Navigate to Roblox and set cookie
        ...
        // 3. Click birthday edit button (pencil icon)
        log('Clicking birthday edit...', logs);
        let birthdayClicked = await page.evaluate(() => {
            // Search all text nodes for "Birthday" then find nearest button
            const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
            let node;
            while (node = walker.nextNode()) {
                if (node.textContent.trim() === 'Birthday') {
        ...
        if (!birthdayClicked) {
            // Fallback selectors
            for (const sel of ['[class*="birthday"] button', '[id*="birthday"] ~ button', '.account-info button']) {
                try { await page.click(sel); birthdayClicked = true; break; } catch (e) {}
            }
        }
